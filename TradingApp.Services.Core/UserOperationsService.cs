...
namespace TradingApp.Services.Core
{
    public class UserOperationsService : IUserOperationsService
    {
        ...
        public async Task<Result> ManageUserAsync(ManagedUserModel user)
        {
            ...
            //checking the range of the days for which the user will be suspended (value 0 means the user is not suspended)
            bool isDaysToSuspendInBoundaries = user.DaysToSuspend >= EntityValidation.User.MinDaysToSuspendUser && user.DaysToSuspend <= EntityValidation.User.MaxDaysToSuspendUser;
            if (isDaysToSuspendInBoundaries == false)
            { return new Result(errorCode: UserErrorCodes.UserDaysToSuspendOutOfBoundaries); }
            ...
            //checking whether there is a lockout message if the user was banned or suspended
            if ((user.DaysToSuspend > 0 || user.IsBanned == true) && string.IsNullOrEmpty(user.LockoutMessage))
            ...
            userFromDB.LockoutMessage = (user.DaysToSuspend > 0 || user.IsBanned == true) ? user.LockoutMessage: null;
            ...
            userFromDB.Banned = user.IsBanned;
            //update DB records>
